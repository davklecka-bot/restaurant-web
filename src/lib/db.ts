...
export async function getPendingEmails() {
  const client = sb()
  const { data: pending } = await client.from('outreach').select('id, restaurant_id').is('email_sent_at', null)
  if (!pending?.length) return []
  const ids = pending.map((p: any) => p.restaurant_id)
  const { data: restaurants } = await client.from('restaurants').select('*').in('id', ids).not('email', 'is', null)
...
export async function markEmailSent(outreachId: number) {
  await sb().from('outreach').update({ email_sent_at: new Date().toISOString(), status: 'sent' }).eq('id', outreachId)
}
